...
최소 참고용 매니페스트가 `manifests/` 아래에 있습니다. 플레이스홀더 값과 Secret 참조만 사용하며, Helm 차트의 보안 기본값(비-root, 읽기 전용 루트 파일시스템, 모든 capability 드롭, `RuntimeDefault` seccomp, 리소스 requests/limits)과 리더 선출 Lease RBAC를 그대로 반영합니다. 완전히 설정 가능한 권위 있는 산출물은 Helm 차트입니다.
...
`make image`는 정적 Linux 바이너리를 빌드하고 `gcr.io/distroless/static-debian12:nonroot` 기반 로컬 Podman 이미지를 만듭니다. 이 이미지는 비-root 사용자로 실행되며 TLS 검증용 CA 인증서를 포함합니다.

`make validate`는 추가로 `make no-workflows`(GitHub Actions 워크플로 파일이 없음을 보장)와 `make secret-scan`(추적 중인 파일에서 커밋된 API 토큰을 스캔)을 실행합니다.
